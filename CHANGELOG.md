...
- feat: 교체 투입/아웃 두 시점 분리 및 재교체 지원

### 변경

- refactor: 최근경기·평점화면 교체표시를 IN/OUT 기준으로 변경
- test: 재교체 시나리오 테스트 추가 및 mock 스키마 갱신
- docs: v2.1.2 교체기록 투입/아웃 분리 버전 정리

### 수정

- fix: 교체 투입 시각 누락 및 재교체 미기록 라인업 데이터 보정
...
- docs: v1.9.2작업내용문서
- build: 타입체크 명령어 통일 및 husky과정적용

### 수정

- fix: 차트 라벨 타입미지정 에러 수정
- fix: husky pre-commit deprecated예정 경고문구 반영
...
- feat: 로딩 컴포넌트 소스원천 단일화
- feat: 페이지 로딩 컴포넌트 구현

### 변경

- refactor: 404페이지 리디자인
- test: 404페이지 테스트추가

### 수정

- fix: 아이폰5 뷰포트기준 메뉴아이템 overflow방지를위한 가로스크롤컴포넌트도입처리
...
> 선수 이적시장 기능 추가 (어드민 CRUD + 공개 화면, 유로 금액 표기, 좋아요/싫어요 반응)

### 추가

- feat: 이적시장 엔터티 및 유로 단위 표현 유틸함수 추가
- feat: 이적시장 어드민 관리 crud 추가
- feat: 선수 이적시장 유저용 화면 구현
- feat: 이적시장 관련 라우팅 경로 추가

### 변경

- chore: 스토리북에 supabase 이미지경로 설정

### 수정

- fix: 스토리북에쓰였던 더미 이미지 경로 교체
...
**Branch**: `bkds-v1.7.1-자동직관체크로직변경및유저랭킹조회기능추가`

### 추가

- feat: 평점입력시 띠부띠부씰 자동직관체크로 기능변경
- feat: 유저랭킹 route주소 등록
- feat: 유저랭킹 조회페이지 개발

### 변경

- refactor: 에러폴백컴포넌트들 에러상황이미지 일괄추가
...
**Branch**: `bdks-v1.6.10-내정보화면디자인수정`

### 추가

- feat: 프로필 수정기능,페이지 추가
- feat: 익명로그인유저 프로필수정불가기능,테스트코드추가

### 변경

- docs: 3d아이콘디자인표준색상변경
- style: 바로가기메뉴 ring 형태 border추가
- style: 디자인변경으로인한 그리드컴포넌트들 삭제
- style: 보유포인트만제공 -> 포인트,닉네임,프로필이미지제공
- style: 디자인변경으로인한 greet컴포넌트삭제

### 수정

- fix: 바로가기 아이콘 순서, 워딩변경
...
- refactor: admin api관련 toast 일괄추가
...
- docs: v1.6.3작업내용문서
- feat: 경기관리자테이블에라이브컬럼보이도록추가
- feat: 경기관람체크,알람신청,미관람상태렌더링
- feat: 관람체크클릭시포인트지급기능RPC+트리거기능활용하여추가

### 변경

- test: 관람체크,미관람,알람신청상태 테스트코드추가

### 수정

- fix: 출석체크->관람체크에맞는네이밍수정
- fix: 평점입력시 대쉬보드화면 평점갱신을 위하여 쿼리키 invalidate화
