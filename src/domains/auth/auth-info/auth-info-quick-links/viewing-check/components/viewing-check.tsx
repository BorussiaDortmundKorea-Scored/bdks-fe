 ...
 * 작성자: KYD
 * 기능: 직관 띠부띠부씰 도감 UI
 * 프로세스 설명: 평점을 입력한 경기는 컬러로 활성화되고 미입력 경기는 회색으로 표시
 */
...
const ViewingCard = ({ match }: IViewingCardProps) => {
  ...
  return (
    <div
      className={`group relative flex flex-col overflow-hidden rounded-lg border-2 transition-all ${
        isViewed ? "border-primary-400 bg-background-tertiary" : "bg-background-tertiary border-black"
      }`}
      aria-label={`${match.opponent_team_name} - ${isViewed ? "관람완료" : "미관람"}`}
    >
      {/* 팀 로고 이미지 */}
      ...
      {/* 카드 하단 정보 */}
      <div className={`w-full pb-2 ${isViewed ? "text-white" : "text-black"} text-center`}>
    ...
    <ViewingCheckWrapper>
      ...
      {matches.length === 0 ? (
        ...
        <div className="grid grid-cols-3 gap-2" data-testid="viewing-check-grid">
          {matches.map((match) => (
            <ViewingCard key={match.id} match={match} />
          ))}
        </div>
      )}
    </ViewingCheckWrapper>
