...
describe("useDeleteTeam", () => {
  ...
  it("성공 시 팀 목록 쿼리를 무효화하고 성공 토스트를 띄운다", async () => {
    deleteTeamMock.mockResolvedValue({ data: true, error: null });
    const client = new QueryClient({ defaultOptions: { mutations: { retry: false } } });
    const invalidateSpy = vi.spyOn(client, "invalidateQueries");

    const { result } = renderHook(() => useDeleteTeam(), { wrapper: createWrapper(client) });
  ...
  it("실패 시 무효화하지 않고 실패 토스트를 띄운다", async () => {
    ...
    expect(toastMock).toHaveBeenCalledWith({ content: "팀 삭제에 실패했어요" });
  });
});
