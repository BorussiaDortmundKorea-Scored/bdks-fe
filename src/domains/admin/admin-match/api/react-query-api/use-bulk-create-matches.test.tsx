...
import { bulkCreateMatches } from "@admin/admin-match/api/admin-match-api";
...
const bulkCreateMatchesMock = bulkCreateMatches as Mock;
...
describe("useBulkCreateMatches", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("성공 시 경기 목록 쿼리를 무효화하고 성공 토스트를 띄운다", async () => {
    bulkCreateMatchesMock.mockResolvedValue({ data: [{ id: "m1" }], error: null });
    const client = new QueryClient({ defaultOptions: { mutations: { retry: false } } });
    const invalidateSpy = vi.spyOn(client, "invalidateQueries");

    const { result } = renderHook(() => useBulkCreateMatches(), { wrapper: createWrapper(client) });
    result.current.mutate(request);

    await waitFor(() => expect(result.current.isSuccess).toBe(true));

    expect(invalidateSpy).toHaveBeenCalledWith({ queryKey: ADMIN_MATCH_QUERY_KEYS.lists() });
    expect(toastMock).toHaveBeenCalledWith({ content: "경기 일괄 생성을 성공했어요" });
  });

  it("실패 시 무효화하지 않고 실패 토스트를 띄운다", async () => {
    const error = { name: "PostgrestError", message: "실패", details: "", hint: "", code: "500" } as PostgrestError;
    bulkCreateMatchesMock.mockResolvedValue({ data: null, error });
    const client = new QueryClient({ defaultOptions: { mutations: { retry: false } } });
    const invalidateSpy = vi.spyOn(client, "invalidateQueries");
    ...
    expect(toastMock).toHaveBeenCalledWith({ content: "경기 일괄 생성에 실패했어요" });
  });
});
