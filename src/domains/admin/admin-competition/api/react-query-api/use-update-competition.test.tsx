...
import { updateCompetition } from "@admin/admin-competition/api/admin-competition-api";
...
const updateCompetitionMock = updateCompetition as Mock;
...
describe("useUpdateCompetition", () => {
  ...
  it("성공 시 대회 쿼리를 무효화하고 성공 토스트를 띄운다", async () => {
    updateCompetitionMock.mockResolvedValue({ data: { id: "c1" }, error: null });
    const client = new QueryClient({ defaultOptions: { mutations: { retry: false } } });
    const invalidateSpy = vi.spyOn(client, "invalidateQueries");

    const { result } = renderHook(() => useUpdateCompetition(), { wrapper: createWrapper(client) });
  ...
  it("실패 시 무효화하지 않고 실패 토스트를 띄운다", async () => {
    ...
    expect(toastMock).toHaveBeenCalledWith({ content: "대회 수정에 실패했어요" });
  });
});
